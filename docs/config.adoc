...
Since the configuration is extensive, documentation has been broken down into several sections below. All available configuration options <<#all-options,listed here>>.
...
IMPORTANT: For ease of reading we omit the `idprovider.<name>.` prefix in most of the documentation below. See the <<examples#examples,examples>> for a complete setup.
...
NOTE: You may disable Authorization Code Flow by omitting `clientId` in your configuration
...
- **Entra**: https://login.microsoftonline.com/`TenantID`/v2.0/.well-known/openid-configuration
...
NOTE: The feature is disabled by default. Presence of `groups.claim` is what enables it. Existing configurations are unaffected.
...
groups.createGroups=(true|false, optional, defaults to true) // <5>
----
...
<5> `createGroups`, when `true`, auto-creates any group referenced in the mapping that does not yet exist. When `false`, missing groups are skipped with a warning.

TIP: For multiple groups, add more lines and iterate the array counter (`groups.mapping.1.value`, `groups.mapping.1.group`, ...).
...
|Okta |`groups` (add the `groups` scope and a claim with a filter in a custom Authorization Server)
|Microsoft Entra ID |`groups` (configured via `groupMembershipClaims` in the app manifest; values are GUIDs by default, so map each GUID to a group key)
|Auth0 |a namespaced URI set by a post-login Action, e.g. `https://app.example.com/groups`
|Keycloak |`realm_access.roles` (built-in), or `groups` with a Group Membership mapper
|===

NOTE: Microsoft Entra emits a "groups overage" indirection instead of the group list for users in more than 200 groups. Resolving it requires a Graph API call and is not handled by this app; such tokens result in no group changes.
...
<5> When `true`, also apply <<#groups-from-claim,groups from claim>> sync during Bearer autoLogin. The claim is resolved against the raw JWT payload (no userinfo endpoint is called).

== endSession

OpenID Connect Front-Channel Logout is optional and might not be supported by your authentication server. You can check if the endpoint is available in the OpenID Configuration (`.well-known/openid-configuration`) under the field `end_session_endpoint`.
...
Microsoft Entra::
* End Session URL: https://login.microsoftonline.com/common/oauth2/v2.0/logout
...
<1> `userEventPrefix` is used to customize the type of user events such as `login`, `create`, and `modify`.
By default, the application name is used as the prefix. For instance, the login event will look like `<userEventPrefix>.user.login`. To listen for this event, use `custom.<userEventPrefix>.user.login`.
...
idprovider.<name>.groups.createGroups=(true|false, defaults to true)
