...
<2> Set this if `useUserinfo` is false.`claimUsername` is important, as this represents the unique identifier that will keep the Enonic user and the OIDC user in sync.  For instance, OIDC uses `sub` as a unique, unchanging user identifier, and in Azure AD this comes from the user's `oid`.
...
This application supports automatically logging in users when the client passes the a special header: `Authorization: Bearer <token>` - The token must be a valid JWT for your ID provider
