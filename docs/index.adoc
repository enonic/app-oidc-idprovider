...
. The Client ID. Not required authentication is done only by Auto Login.
. The Client secret. Not required authentication is done only by Auto Login.
. The Issuer Identifier (URL)
. The Authorization endpoint URL
. The Token endpoint URL
. The JWKS URI
. The `oidcWellKnownEndpoint` (Provider's OpenID Connect Discovery URL), to obtain the values for the above fields and for `userinfoUrl`.
...
The JSON result will contain the additional information: `issuer`, `authorization_endpoint`, `token_endpoint` and `jwks_uri`.
...
* `usePkce`: Use PKCE. If set to `true`, the app will use PKCE for the Authorization Code Flow.
...
To disable the ability to auth by Authorization Code Flow, `clientId` should be omitted.
...
- The `clientId` and `clientSecret` fields are now optional in the ID Provider configuration. If you want to disable the ability to auth by Authorization Code Flow, `clientId` should be omitted.
