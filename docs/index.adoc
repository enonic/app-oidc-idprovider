...
. The Client ID. Must be set, if log in via form is enabled.
. The Client secret. Must be set, if log in via form is enabled.
. The Issuer Identifier (URL)
. The Authorization endpoint URL
. The Token endpoint URL
. The JWKS URI
. The `oidcWellKnownEndpoint` (OpenID Provider Configuration URL), to obtain the values for the above fields and for `userinfoUrl`.
...
The JSON result will contain the required information: `issuer`, `authorization_endpoint`, `token_endpoint` and `jwks_uri`.
...
idprovider.<idprovidername>.jwksUri=  (string, required)
idprovider.<idprovidername>.useUserinfo= (true|false, optional, default to "true")
idprovider.<idprovidername>.claimUsername= (string, optional, default to "sub")
...
idprovider.<idprovidername>.mappings.displayName=  (string, required, default to ${userinfo.preferred_username})
idprovider.<idprovidername>.mappings.email=  (string, required, default to ${userinfo.email})
...
idprovider.<idprovidername>.autoLogin.createUsers=  (true|false, optional, default to "true")
idprovider.<idprovidername>.autoLogin.createSession=  (true|false, optional, default to "false")
...
* `displayName`: Display name of ID Provider in User app.
* `description`: Description of ID Provider in User app.
* `useUserinfo`: Use the Userinfo endpoint to retrieve user claims.
* `claimUsername`: Claim to use as username. The value is a string, the default value is `sub`.

**Authorization Server**

* `oidcWellKnownEndpoint`: OpenID Provider Configuration URL. If defined, then values for the `issuer`, `authorizationUrl`,`tokenUrl`,`userinfoUrl`,`jwksUri` properties will be fetched from the well-known endpoint.
...
* `userinfoUrl`: Userinfo endpoint URL. Value of `userinfo_endpoint` in your OpenID Provider configuration.
* `jwksUri`: JSON Web Key Set (JWKS) endpoint URL. Value of `jwks_uri` in your OpenID Provider configuration.
...
* `autoLogin...` (`idprovider.<idprovidername>.autoLogin...` namespace): Auto login configuration.
  ** `createUsers`: Create users automatically. If set to `true`, users will be created automatically if they don't exist.
  ** `createSession`: Automatically create a session. If set to `true`, a session will be established upon the user's login. By default, the user will be logged in with `REQUEST` scope.
  ...
  ** `allowedAudience`: Allowed audience. A list of space-separated strings. If set, the app will only accept tokens with an audience that matches one of the values in this list.

To disable the ability to log in via the form, `clientId` and `clientSecret` must be omitted.
...
Version `3.0` of this app introduces a breaking change:

- The `jwksUri` field is now required in the ID Provider configuration to verify the token signature.
- The `clientId` and `clientSecret` fields are now optional in the ID Provider configuration. If you want to disable the ability to log in via the form, these fields must be omitted.
