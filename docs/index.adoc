...
idprovider.<idprovidername>.autoLogin.createUser=  (true|false, optional, default to "true")
...
* `autoLogin...` (`idprovider.<idprovidername>.autoLogin...` namespace): Auto login configuration. To enable auto login, the `jwksUri` config must be set.
  ** `createUser`: Create a user. If set to `true`, a user will be created automatically if they don't exist.
...
- To verify the token signature the `jwksUri` field is now required in the ID Provider configuration.
